...
struct _sapi_module_struct {
	...
	char *pretty_name;				//更容易理解的名字，如fpm-fcgi对应的FPM/FastCGI

	int (*startup)(struct _sapi_module_struct *sapi_module);	//模块启动时候调用的函数
	int (*shutdown)(struct _sapi_module_struct *sapi_module);	//模块结束时候调用的函数
	...
	int php_ini_ignore;											//是否不适用任何ini配置文件
	int php_ini_ignore_cwd; /* don't look for php.ini in the current directory */ //忽略当前路径的php.ini
	...
	//对输入进行过滤的函数指针。比如将输入函数填充到自动全局变量$_GET,$_POST,$_COOKIE中
	...
	char *ini_entries;											//执行时附带的ini配置，可以使用php -d设置
 	const zend_function_entry *additional_functions;			//每个SAPI模块特有的一些注册函数，比如cli的cli_get_process_title
