...
struct _zval_struct {
	...
	union {
		struct {
			ZEND_ENDIAN_LOHI_4(
				zend_uchar    type,			/* active type */					//标明zval类型
	...
	union {
		uint32_t     next;                 /* hash collision chain */			//用来解决哈希冲突，记录冲突的下一个元素位置
		uint32_t     cache_slot;           /* literal cache slot */				//运行时缓存
		uint32_t     lineno;               /* line number (for ast nodes) */	//文件执行的行号
		uint32_t     num_args;             /* arguments number for EX(This) */	//函数调用时传入参数的个数
		uint32_t     fe_pos;               /* foreach position */				//foreach的位置
...
struct _zend_array {
	...
	uint32_t          nTableMask;		//掩码。一般为-nTableSize
	Bucket           *arData;			//实际的存储容器。通过指针指向一段连续的内存，存储着bucket数组
	uint32_t          nNumUsed;			//指所有已使用bucket数量，包括有效Bucket和无效bucket
	uint32_t          nNumOfElements;	//有效bucket数量。该值总是小于等于nNumUsed
	uint32_t          nTableSize;		//HashTable的大小。表示arData指向的bucket数组的大小，即所有buckcet的数量。该字段取值始终是2^n,最小值时候8
	uint32_t          nInternalPointer; //全局默认游标
	zend_long         nNextFreeElement; //HashTable的自然key。自然key是指HashTable的应用语义是纯数组时，插入元素无须指定key，key会以nNextFreeElement的值为准
...
struct _zend_object {
	...
	uint32_t          handle; // TODO: may be removed ???	//每生成一个结构体zend_object,会将其首地址存储在全局变量中，这个就是索引
	zend_class_entry *ce;									//所属的类结构体指针
	const zend_object_handlers *handlers;					//初始化时，默认指向全局变量std_object_handlers，存储着包括操作对象属性等的多个指针函数
