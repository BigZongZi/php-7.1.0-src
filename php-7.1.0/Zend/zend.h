...
struct _zend_class_entry {
	char type;								//类的类型，共有两种 1.代表内置的类 2.代表用户自定义的类
	zend_string *name;						//类名
	...
	zval *static_members_table;				//静态属性成员
	...
	union _zend_function *__get;			//魔术方法__get
	union _zend_function *__set;			//魔术方法__set
	union _zend_function *__unset;			//魔术方法__unset
	union _zend_function *__isset;
	union _zend_function *__call;
	union _zend_function *__callstatic;
	union _zend_function *__tostring;
	union _zend_function *__debugInfo;
	union _zend_function *serialize_func;	//对象序列化方法
	union _zend_function *unserialize_func;	//对象反序列化方法

	zend_class_iterator_funcs iterator_funcs;//php5开始，支持接口并且内置了Iterator接口，其为此接口的相关操作方法

	/* handlers */
	zend_object* (*create_object)(zend_class_entry *class_type);	//实例化对象时调用的方法，默认为函数zend_objects_new
	zend_object_iterator *(*get_iterator)(zend_class_entry *ce, zval *object, int by_ref);
	int (*interface_gets_implemented)(zend_class_entry *iface, zend_class_entry *class_type); /* a class implements this interface */
	union _zend_function *(*get_static_method)(zend_class_entry *ce, zend_string* method);

	/* serializer callbacks */
	int (*serialize)(zval *object, unsigned char **buffer, size_t *buf_len, zend_serialize_data *data);//序列化方法回调指针
	int (*unserialize)(zval *object, zend_class_entry *ce, const unsigned char *buf, size_t buf_len, zend_unserialize_data *data);	//反序列化方法回调指针

	uint32_t num_interfaces;				//类implements的接口个数
	uint32_t num_traits;					//类use的特性个数
	zend_class_entry **interfaces;			//类implememts的接口指针

	zend_class_entry **traits;				//类use的traits指针
	zend_trait_alias **trait_aliases;		//类use的特性方法的别名
	zend_trait_precedence **trait_precedences;//类use的特性方法的优先级（用于多个特性有相同名称的方法时，解决冲突）
